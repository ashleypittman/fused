...
{
	fuse_session_process_buf_chan(se, buf, NULL);
}

void fuse_session_process_buf_chan(struct fuse_session *se,
...
{
	return fuse_session_receive_buf_chan(se, buf, NULL);
}

int fuse_session_receive_buf_chan(struct fuse_session *se, struct fuse_buf *buf,
				 struct fuse_chan *ch)
{
	int err;
	ssize_t res;
